...
export function Devices() {
  ...
  const [form, setForm] = useState({ name: '', branch: '', location: '', model: '', ipAddress: '' });

  const resetForm = () => setForm({ name: '', branch: '', location: '', model: '', ipAddress: '' });
  const openAdd = () => { resetForm(); setIsAddDialogOpen(true); };
  const openEdit = (d: Device) => { setForm({ name: d.name, branch: d.branch, location: d.location, model: d.model, ipAddress: d.ipAddress }); setEditDevice(d); };

  const handleAdd = () => {
    if (!form.name) return;
    setDevices(prev => [{ id: Date.now().toString(), name: form.name, branch: form.branch || 'Headquarters', location: form.location, status: 'online' as const, lastSeen: new Date().toISOString(), lastActivity: new Date().toISOString(), ipAddress: form.ipAddress, firmwareVersion: 'v2.1.0', todayCount: 0, model: form.model || 'FaceID Pro X1' }, ...prev]);
    setIsAddDialogOpen(false); resetForm();
  };
  const handleEdit = () => {
    if (!editDevice || !form.name) return;
    setDevices(prev => prev.map(d => d.id === editDevice.id ? { ...d, name: form.name, branch: form.branch || d.branch, location: form.location, model: form.model || d.model, ipAddress: form.ipAddress } : d));
